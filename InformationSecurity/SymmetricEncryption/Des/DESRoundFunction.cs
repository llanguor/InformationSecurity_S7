    ...
    : IRoundFunction
{
	...
	/// <summary>
	/// Expansion permutation table (E) to expand a 32-bit half-block to 48 bits.
	/// </summary>
    private static readonly int[] ExpansionPermutation = 
    [
        32, 1, 2, 3, 4, 5,
        4, 5, 6, 7, 8, 9,
        8, 9, 10, 11, 12, 13,
	...
	/// <summary>
	/// Permutation (P) table applied after S-box substitution.
	/// </summary>
    private static readonly int[] PrimitivePermutation =
    [
	    16, 7, 20, 21,
	    29, 12, 28, 17,
	    1, 15, 23, 26,
	...
	/// <summary>
	/// The eight DES S-boxes used to compress the 48-bit input to 32 bits.
	/// </summary>
    ...
    /// <summary>
    /// Transforms a 64-bit block using the DES round function (F-function).
    /// </summary>
    /// <param name="block">
    ...
    public void TransformBlock(Span<byte> block, ReadOnlySpan<byte> key)
    {
        ...
        ref var ulongResult =
	        ref MemoryMarshal.AsRef<ulong>(buffer);
        
        ulongResult >>= 16;
	    for (var i = 0; i < 8; ++i)
	    {
		    var row = ulongResult & 0b000001 | (ulongResult & 0b100000) >> 4;
		    var col = (ulongResult & 0b011110)>>1;
